...
| 사용 가능 메뉴    | menu.explicit      | DASHBOARD, CUSTOMER, TENANT | 관리자도구에 기본 제공되는 메뉴 대신, 명시적으로 사용할 메뉴를 설정하면 해당 메뉴만 표시됩니다. 입력할 수 있는 문자열은 SectionType 클래스에 정의된 SectionType 값 입니다. 아무 설정이 없는 경우 전체 메뉴를 모두 표시합니다. |
